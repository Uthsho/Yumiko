...
module.exports.run = async(client, message, args, prefix) => {
  ...
  if(!args[0]) {
    ...
    .addField(modemoji + " Moderation - (12)", "`config`, `announce`, `warn`, `warnings`, `menro`,  `setprefix`, `poll`, \n `kick`, `ban`, `role`, `purge`, `avatar`, `lockdown`, `swear`", true)
    .addField(utilityemoji + " Utility - (5)", "`ping`, `snipe`, `help`, `suggest`, `invite`", true)
    ...
    .addField(shortcut + " Shortcut - (7)", "`mention-role`, `av`, `addcc`, `removecc`, `prefix`, `sprefix`, `userinfo`", true)
    .addField(`Note`, `If backup command doesn't work report to the staff team immediately to reenable it again!`)
    .addField(`${updateemoji} Update:`, `Made a anti-swear system!. Made a custom command/response system!. Made a config command to see server config. Made a backup command. \n SneakPeak: Working on a rpg system!`)
    .addField("\u200b", `[SUPPORT](https://discord.gg/N6Kwxd) - [INVITE](https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=2146958847%60)`)
